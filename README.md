...
你找到那栋，GitHub库发布到iUAP文件中心在线技术文档的源http://iuap.yonyou.com文件。
...
有助于论坛讨论。
在文章的底部提交Disqus评论。
你可以很容易的在GitHub的用户界面技术文章投稿。要么找到该知识库中的文章，或访问的文章http：／／azure.microsoft.com /文件点击文章到文章的GitHub源链接。
如果你是一个现有的文章作出实质性的改变，增加或改变的图像，或者贡献一个新的文章，你需要叉这个库，安装Git Bash，降价垫，并学习一些Git命令。
你蔚蓝的贡献内容

轻微的修正

小的更正或澄清你提交的文档和代码在这个回购的例子是由Azure的网站使用条款（头）。
...
微软的开放技术组成员。
人谁不工作的微软。
我们需要你在我们可以接受你的要求完成在线形式拉。

详情可在http：/ / / / # CLA azure.github.io指南。

库的组织

在Azure的内容库的内容包括文件的组织azure.microsoft.com。这个库包含了两根文件夹：

\第

这个\第文件夹包含文件的文章格式为一个Markdown文件。MD扩展

在根目录中的文章发布到azure.microsoft.com路径中azure.microsoft.com http：/ / / /文章/文件名称没有软腭{ } / MD。

文章的文件名：看到我们的文件命名指导。
在自己的工作文件夹中的文章发布到azure.microsoft.com路径中http：／／azure.microsoft.com /文件/文件夹/物品/服务名称没有软腭{ } / MD

媒体文件夹：这个\第文件夹包含\媒体根目录第媒体文件的文件夹，里面的每一篇文章都有图片文件夹。服务目录包含一个独立的媒体文件夹中的文章在每个服务文件夹。本文的图像文件夹的命名与文章文件，减去。MD文件扩展名。
\包括

您可以创建可重用内容的部分被包含在一个或更多的文章。看到用我们的技术内容的自定义扩展。
...
有关如何贡献，如何使用GitHub的UI贡献小的变化，以及如何叉和克隆更多的重大贡献的仓库，看到安装和设置工具创作在GitHub。

如果你安装gitbash选择在本地工作的步骤，创建一个新的地方工作科，做出改变，并将改变回到主分支中列出创建一个新的文章或更新现有的第Git命令

分支

我们建议您创建的本地分支，针对特定范围的变化。各分支机构应限制在一个单一的概念/文章既简化工作流程，减少合并冲突的可能性。下面的工作的一个新的分支是合适的范围：

一个新的文章（相关图片）
拼写和语法编辑一篇文章。
在一个大的物品应用单一格式改变（例如，新的版权页脚）。
如何使用Markdown格式你的话题

该库使用GitHub的所有物品味降价。这里的资源列表。
...
文章的元数据使某些功能在azure.microsoft.com网站，如作者归属，原因归因、面包糠、文章的描述，和SEO优化以及微软使用评价报告内容的表现。因此，元数据是重要的！这是确保你的元数据进行正确引导。

更多的资源

看到我们的贡献者的索引指南我们的指导主题。
